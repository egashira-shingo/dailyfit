# パーソナルジム Daily Fit 公式サイト

## 1. システム概要

このリポジトリは、パーソナルジム「Daily Fit」の公式サイトのソースコードを管理するものです。
...
開発・運用に関するその他の手順書は以下の通りです。

- **[Google Workspace 新規セットアップ手順](./requirements/google_workspace_setup.md):** `dailyfit.jp` ドメインを用いた新規Google Workspaceのアカウント開設、および DNSレコード（TXT, MX, SPF, DKIM）の設定手順
- **[GBP (Google Business Profile) 移行・統合に関する見解](./requirements/gbp_second_opinion.md):** 既存のGBPアカウントから新アカウントへの安全な移行、およびリブランディングに関するリスクと対策のまとめ
