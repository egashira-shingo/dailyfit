...
- **[Google Workspace 新規セットアップ手順](./google_workspace_setup.md):** `dailyfit.jp` ドメインを用いた新規Google Workspaceのアカウント開設、および DNSレコード（TXT, MX, SPF, DKIM）の設定手順
- **[GBP (Google Business Profile) 移行・統合に関する見解](./gbp_second_opinion.md):** 既存のGBPアカウントから新アカウントへの安全な移行、およびリブランディングに関するリスクと対策のまとめ
